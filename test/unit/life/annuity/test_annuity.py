...
def test_interest_rate(discount_factors, expected):
    assert np.allclose(ann.interest_rate(discount_factors), expected, atol=1e-03)
...
def test_discount_rate(interest_rates, expected):
    assert np.allclose(ann.discount_rate(interest_rates), expected, atol=1e-03)
...
def test_annuity_pv(periods, interest_rates, expected):
    assert np.allclose(
        ann.annuity_pv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_annuity_due_pv(periods, interest_rates, expected):
    assert np.allclose(
        ann.annuity_due_pv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_perpetuity_pv(interest_rates, expected):
    assert np.allclose(ann.perpetuity_pv(i=interest_rates), expected, atol=1e-03)
...
def test_perpetuity_due_pv(interest_rates, expected):
    assert np.allclose(ann.perpetuity_due_pv(i=interest_rates), expected, atol=1e-03)
...
def test_annuity_fv(periods, interest_rates, expected):
    assert np.allclose(
        ann.annuity_fv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_increasing_annuity_pv(periods, interest_rates, expected):
    assert np.allclose(
        ann.increasing_annuity_pv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_increasing_annuity_due_pv(periods, interest_rates, expected):
    assert np.allclose(
        ann.increasing_annuity_due_pv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_increasing_annuity_fv(periods, interest_rates, expected):
    assert np.allclose(
        ann.increasing_annuity_fv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_increasing_annuity_due_fv(periods, interest_rates, expected):
    assert np.allclose(
        ann.increasing_annuity_due_fv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_decreasing_annuity_pv(periods, interest_rates, expected):
    assert np.allclose(
        ann.decreasing_annuity_pv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_decreasing_annuity_due_pv(periods, interest_rates, expected):
    assert np.allclose(
        ann.decreasing_annuity_due_pv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_decreasing_annuity_fv(periods, interest_rates, expected):
    assert np.allclose(
        ann.decreasing_annuity_fv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_decreasing_annuity_due_fv(periods, interest_rates, expected):
    assert np.allclose(
        ann.decreasing_annuity_due_fv(n=periods, i=interest_rates), expected, atol=1e-03
    )
...
def test_geo_increasing_annuity_pv(
    ...
    assert np.allclose(
        ann.geo_increasing_annuity_pv(
            n=periods, i=interest_rates, k=payment_growth_rates
        ),
        expected,
        atol=1e-03,
    )
